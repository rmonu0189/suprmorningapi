...
final class SubscriptionRepository
{
    ...
    {
        $stmt = Database::connection()->prepare(
            'SELECT s.id, s.user_id, s.variant_id, s.frequency, s.quantity, s.weekly_schedule, s.start_date, s.pause_start_date, s.pause_end_date, s.created_at,
    ...
    public static function updatePauseByIdForUser(
        string $id,
        string $userId,
        ...
        $stmt = Database::connection()->prepare(
            'UPDATE subscriptions
             SET pause_start_date = :pause_start_date, pause_end_date = :pause_end_date
             WHERE id = :id AND user_id = :uid'
        );
        $stmt->execute([
            'id' => $id,
    ...
    {
        $sql = 'SELECT s.id, s.user_id, s.variant_id, s.frequency, s.quantity, s.weekly_schedule, s.start_date, s.pause_start_date, s.pause_end_date, s.created_at,
                       u.phone AS user_phone, u.email AS user_email, u.full_name AS user_full_name,
                       p.name AS product_name, b.name AS brand_name, v.name AS variant_label, v.sku AS variant_sku, v.images AS variant_images, v.price AS variant_price, v.mrp AS variant_mrp
                FROM subscriptions s
                LEFT JOIN users u ON u.id = s.user_id
    ...
    private static function normalizeRow(array $row): array
    {
        ...
        return [
            ...
            'pause_start_date' => isset($row['pause_start_date']) ? (string) $row['pause_start_date'] : null,
            'pause_end_date' => isset($row['pause_end_date']) ? (string) $row['pause_end_date'] : null,
